use crate::{Accept, Accepted, Byte, Class, Config, HaystackByte, NeedleByte};
use std::ops::Range;
...
pub struct Fuzzy {
    config: Config,
    /// Set of bytes in the needle.
    needle_set: [bool; 256],
    /// Needle bytes.
    needle_bytes: Vec<NeedleByte>,
    /// Current haystack bytes.
    haystack_bytes: Vec<HaystackByte>,
    /// Current haystack's needle's indices ranges.
    ranges: Vec<Range<usize>>,
    /// Current haystack's best needle's indices ranges.
...
impl Fuzzy {
    ...
    pub fn matches<'a>(
        ...
        Byte::parse(needle, |_, byte, is_start| {
            self.needle_set[byte.byte as usize] = true;
            self.needle_bytes.push(NeedleByte {
                byte: byte.byte,
                is_start,
        ...
        for (index, haystack) in haystacks.into_iter().enumerate() {
            if let Some((score, indices)) = self.compute_one(haystack) {
...
/// Private.
impl Fuzzy {
    fn compute_one(&mut self, haystack: &str) -> Option<(isize, &[Range<usize>])> {
        self.haystack_bytes.clear();
        ...
        emit(
            ...
            &self.haystack_bytes,
            |needle_bytes| {
                let score = score(&self.config, needle_bytes, &mut self.ranges);
